...
class VARXGenerator(SeriesGenerator):
    ...
    def __init__(
        ...
        self.coef_exog = pd.DataFrame(coef_exog, index=exogenous, columns=endogenous)

        self.constants = pd.Series(constants, index=endogenous)
        self.covariance = pd.DataFrame(covariance, index=endogenous, columns=endogenous)
    ...
    def generate(self, index, exog=None, random_state=None, initial_values=None):
        ...
        if initial_values is None:
            initial_values = pd.DataFrame(
                0, index=pd.RangeIndex(-maxlag, 0), columns=self.endogenous
            )
        elif np.isscalar(initial_values):
            initial_values = pd.DataFrame(
                initial_values, index=pd.RangeIndex(-maxlag, 0), columns=self.endogenous
            )
        else:
            initial_values = pd.DataFrame(
                np.array(initial_values), columns=self.endogenous
            )
            initial_values.index = pd.RangeIndex(-len(initial_values), 0)

        res = pd.concat([initial_values, exog_impact + eps], axis="rows")
    ...
    @property
    def params(self):
        """Returns coefficients as a Pandas series."""

        res = pd.Series(name="param")

        for k, v in self.constants.items():
            res["const.{}".format(k)] = v

        # Add AR coeffs
        for lag, mtx in self.coef_ar.items():
            for iv in self.endogenous:
                for jv in self.endogenous:
                    name = "L{}.{}.{}".format(lag, iv, jv)
                    res[name] = mtx.loc[iv, jv]

        # Add exogenous coeffs

        # TODO: Implement!

        # Add covariance matrix
        for i, iv in enumerate(self.endogenous):
            for j, jv in enumerate(self.endogenous):
                if j < i:
                    continue
                if j == i:
                    name = "sqrt.var.{}".format(iv)
                else:
                    name = "sqrt.cov.{}.{}".format(iv, jv)
                val1 = self.covariance.loc[iv, jv]
                res[name] = np.sqrt(np.abs(val1)) * np.sign(val1)
